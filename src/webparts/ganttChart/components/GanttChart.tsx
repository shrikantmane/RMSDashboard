  ...
  private initGanttEvents(): void {
    ...
    gantt.attachEvent('onAfterTaskAdd', (id, task) => {
     ...
     // else {
        ...
        console.log("task.duration", task.parent);

          const updatedTask: IGanttChartItemProp = {
            Title: task.text,
            Body: task.Body,
            // StartDate: moment(task.start_date).format("YYYY-MM-DDTHH:MM:ssZ"),
            StartDate: moment(task.start_date).add(1, 'days').format("YYYY-MM-DDTHH:MM:ssZ"),
            DueDate: moment(task.end_date).format("YYYY-MM-DDTHH:MM:ssZ"),
            // PercentComplete: task.progress,
             PercentComplete: "0",
            // AssignedToId: {
            ...
            //   results: [task.parent] || []
            // },
            PredecessorsId: String(task.parent),
      ...
      // } else {
        let users: number[] = [];
          const updatedTask: IGanttChartItemProp = {
            ...
            StartDate: moment(task.start_date).add(1, 'days').format("YYYY-MM-DDTHH:MM:ssZ"),
            DueDate: moment(task.end_date).format("YYYY-MM-DDTHH:MM:ssZ"),
            PercentComplete: String(task.progress)
            // AssignedToId: {
            //   results: users
            // }
          };
        ...
        .get().then( res => {
          ...
          const data: IGanttData[] = res.map( obj => {
            ...
            let rObj: IGanttData = {
              ...
              start_date: moment(obj.StartDate).subtract(1, 'days').format("DD/MM/YYYY"),
          ...
          //   GanttRes : ganttData
          // }))
          // console.log("GanttRes -",this.state.GanttRes);
          //  resolve(this.state.GanttRes); 
