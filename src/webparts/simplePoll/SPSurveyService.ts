...
export class SPSurveyService implements ISPSurveyService {
  ...
  public submitResponse(surveyListId: string, question: string, choice: string){

    // this.getListName(surveyListId).then((listName: string) =>{
     
    
    // })

    pnp.sp.web.lists.getByTitle('RMS Survey').items.add({    
      question : choice
      });
